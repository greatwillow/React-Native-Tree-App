            ...
            <View style={[styles.innerContainer, innerContainerTransparentStyle]}>
              <CloseIcon {...this.props} onPress = {this._setModalVisible.bind(this, false)} />
                ...
                <Text style={{color: 'black', fontSize: 18, fontWeight: '200', textAlign: 'center', justifyContent: 'center', padding: 20}}>
                  By clicking the button below, {`${this.props.textName}: `} <Text style={{color: 'red'}}>{this.props.name}</Text> and all its accompanying data will be permanently deleted from the list of {`${this.props.textName}'s`}.
                </Text>

                <TouchableOpacity
                  style={commonStyles.modalInternalButton}
                  onPress={() => this._onPressRemoveElementFromList()}
                  ><Text style={{color: 'white', fontSize: 18, fontWeight: '200', alignSelf: 'center', padding: 5}}>{`Remove ${this.props.textName}`}</Text>
                </TouchableOpacity>
            </View>
          </View>
        </Modal>
